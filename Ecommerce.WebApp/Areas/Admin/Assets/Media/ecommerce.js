    ...
    methods: {
        open: function (options) {
            if (options) {
                ...
                this.confirmText = options.confirmText ? options.confirmText : ecommerce.resources.texts.ok;
                this.cancelCss = options.cancelCss ? options.cancelCss : "btn-secondary";
                this.cancelIcon = options.cancelIcon;
                this.cancelText = options.cancelText ? options.cancelText : ecommerce.resources.texts.cancel;
...
permissions = {
    ...
    load: function (cb) {
        ...
        if (!this.loaded) {
            fetch(ecommerce.baseUrl + "manager/api/permissions")
...
utils = {
    formatUrl: function (str) {
        return str.replace("~/", ecommerce.baseUrl);
    },
...
blockpicker = new Vue({
    ...
    methods: {
        open: function (callback, index, parentType) {
            ...
            fetch(ecommerce.baseUrl + "manager/api/content/blocktypes" + (parentType != null ? "/" + parentType : ""))
                .then(function (response) { return response.json(); })
                .then(function (result) {
...
notifications = new Vue({
    ...
    methods: {
        push: function (notification) {
            ...
            ecommerce.notifications.items.push(notification);
            ...
            setTimeout(function () {
                ...
                if (notification.hide)
                {
                    setTimeout(function () {
                        ...
                        setTimeout(function () {
                            ecommerce.notifications.items.shift();
                        }, 200);
                    }, 5000);
                }
            }, 200);
...
mediapicker = new Vue({
    ...
    computed: {
        filteredFolders: function () {
            return this.folders.filter(function (item) {
                if (ecommerce.mediapicker.search.length > 0) {
                    return item.name.toLowerCase().indexOf(ecommerce.mediapicker.search.toLowerCase()) > -1
                }
                return true;
            });
        },
        filteredItems: function () {
            return this.items.filter(function (item) {
                if (ecommerce.mediapicker.search.length > 0) {
                    return item.filename.toLowerCase().indexOf(ecommerce.mediapicker.search.toLowerCase()) > -1
                }
                return true;
            });
        }
    ...
    methods: {
        ...
        load: function (id) {
            ...
            var url = ecommerce.baseUrl + "manager/api/media/list" + (id ? "/" + id : "")+"/?width=210&height=160";
            if (this.filter) {
        ...
        getThumbnailUrl: function (item) {
            return item.altVersionUrl !== null ? item.altVersionUrl : ecommerce.baseUrl + "manager/api/media/url/" + item.id + "/210/160";
        },
        refresh: function () {
            ecommerce.mediapicker.load(ecommerce.mediapicker.currentFolderId);
        },
        ...
        savefolder: function () {
            ...
            if (self.folderName !== "") {
                fetch(ecommerce.baseUrl + "manager/api/media/folder/save" + (self.filter ? "?filter=" + self.filter : ""), {
                    method: "post",
                    headers: {
                ...
                .then(function (result) {
                    ...
                    ecommerce.notifications.push(result.status);
                })
                .catch(function (error) {
                    console.log("error:", error);
                });
    ...
    mounted: function () {
        ecommerce.permissions.load(function () {
            if (ecommerce.permissions.media.add) {
                this.dropzone = ecommerce.dropzone.init("#mediapicker-upload-container");
                this.dropzone.on("complete", function (file) {
                    if (file.status === "success") {
                        setTimeout(function () {
                            ecommerce.mediapicker.dropzone.removeFile(file);
                        }, 3000)
                    }
                });
                this.dropzone.on("queuecomplete", function () {
                    ecommerce.mediapicker.refresh();
                });
            }
        });
    }
...
pagepicker = new Vue({
    ...
    methods: {
        load: function (siteId) {
            var url = ecommerce.baseUrl + "ecommerce/api/page/sitemap" + (siteId ? "/" + siteId : "");
            var self = this;
        ...
        refresh: function () {
            this.load(ecommerce.pagepicker.currentSiteId);
        },
...
postpicker = new Vue({
    ...
    computed: {
        filteredPosts: function () {
            return this.posts.filter(function (post) {
                if (ecommerce.postpicker.search.length > 0) {
                    return post.title.toLowerCase().indexOf(ecommerce.postpicker.search.toLowerCase()) > -1
                }
                return true;
            });
        }
    ...
    methods: {
        load: function (siteId, archiveId) {
            var url = ecommerce.baseUrl + "manager/api/post/modal";
            ...
            fetch(url)
                .then(function (response) { return response.json(); })
                .then(function (result) {
                    ecommerce.postpicker.sites = result.sites;
                    ecommerce.postpicker.archives = result.archives;
                    ecommerce.postpicker.posts = result.posts;

                    ecommerce.postpicker.currentSiteId = result.siteId;
                    ecommerce.postpicker.currentArchiveId = result.archiveId;

                    ecommerce.postpicker.currentSiteTitle = result.siteTitle;
                    ecommerce.postpicker.currentArchiveTitle = result.archiveTitle;
                })
                .catch(function (error) { console.log("error:", error ); });
        },
        refresh: function () {
...
preview = new Vue({
    ...
    methods: {
        openItem: function (media) {
            ecommerce.preview.media = media;
            ecommerce.preview.show();
        },
        //TODO: Rename loadAndOpen?
        open: function (mediaId) {
            ecommerce.preview.load(mediaId);
            ecommerce.preview.show();
        },
        load: function (mediaId) {
            var self = this;

            fetch(ecommerce.baseUrl + "manager/api/media/" + mediaId)
                .then(function (response) { return response.json(); })
                .then(function (result) {
                    self.media = result;
                })
        ...
        saveMeta: function (media) {
            ...
            fetch(ecommerce.baseUrl + "manager/api/media/meta/save", {
                method: "post",
                headers: {
                    "Content-Type": "application/json",
                },
            ...
            .then(function (response) { return response.json(); })
            .then(function (result) {
                ecommerce.notifications.push(result);
    ...
    mounted: function () {
        this.dropzone = ecommerce.dropzone.init("#media-update-container", {
            uploadMultiple: false
        });
        this.dropzone.on("complete", function (file) {
            setTimeout(function () {
                ecommerce.preview.dropzone.removeFile(file);
            }, 3000)
        })
        this.dropzone.on("queuecomplete", function () {
            ecommerce.preview.load(ecommerce.preview.media.id);
            ecommerce.media.refresh();
        })
    }
});
            ...
            toolbar: [
                "bold", "italic", "heading", "|", "quote", "unordered-list", "ordered-list", "|", "link",
                {
                    ...
                    action: function customFunction(editor) {
                        ecommerce.mediapicker.openCurrentFolder(function(media) {
